    ...
    unsafe {
      ...
      match avformat_alloc_output_context2(
        ...
        0 => {
          ...
          let ret =
            avio_open(&mut io, p.as_ptr() as *const i8, AVIO_FLAG_WRITE);
          if ret < 0 {
            panic!("open ouput file {} failed", path);
          }
          (*context).pb = io;
          Box::new(AvformatMuxer { context, duration: 0 })
        }
...
impl Muxer for AvformatMuxer {
  fn write_header(
    ...
    unsafe {
      ...
      let framerate_den = framerate_den as i32;
      let framerate_num = framerate_num as i32;
      let duration = (*stream).time_base.den * framerate_den
        / ((*stream).time_base.num * framerate_num);
  ...
  fn write_frame(&mut self, pts: u64, data: &[u8]) {
    unsafe {
      ...
      let pts = pts as i64;
      pkt.pts = pts * self.duration;
      //no b frame, pts equals dts
      pkt.dts = pts * self.duration;
